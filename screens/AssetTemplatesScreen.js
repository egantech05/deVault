...
import { AddCard, DisplayCard } from "../components/Cards"; // <-- use shared cards
...
export default function AssetTemplatesScreen() {
  ...
  const listKey = useMemo(
    () => `asset-templates-cols-${numColumns}`,
    [numColumns]
  );
      ...
      <FlatList
        ...
        renderItem={({ item }) =>
          ...
          <DisplayCard
            ...
            borderColor="#E5E7EB"
            // stretch children so header can reach the edges
            style={{ alignItems: "stretch" }}
            onPress={() => openDetails(item)}
          >
            {/* Fill the whole tile so we can push title to the bottom */}
            <View style={{ flex: 1 }}>
              {/* TOP-RIGHT COUNT */}
              <View style={{ alignSelf: "stretch", flexDirection: "row", justifyContent: "flex-end" }}>
                <View style={stylesLocal.countPill}>
                  <Text style={stylesLocal.countText}>{item.assetCount ?? 0}</Text>
                </View>
              </View>
