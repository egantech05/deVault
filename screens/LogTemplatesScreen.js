...
import { View, Text, FlatList, useWindowDimensions, Pressable, Alert, StyleSheet } from "react-native";
...
export default function LogTemplatesScreen() {
  ...
  // Create modal fields
  const [properties, setProperties] = useState([{ id: 1, name: "", property_type: "text", default_value: "" }]);
  ...
  const isDuplicateName = !!normalizedNewName && templates.some(t => (t.name || "").toLowerCase() === normalizedNewName);
  ...
  const addProperty = () => setProperties(prev => [...prev, { id: nextIdRef.current++, name: "", property_type: "text", default_value: "" }]);
  const removeProperty = (id) => { if (properties.length > 1) setProperties(prev => prev.filter(p => p.id !== id)); };
  const updateProperty = (id, field, value) => setProperties(prev => prev.map(p => p.id === id ? { ...p, [field]: value } : p));
  ...
  const handleAddTemplate = async () => {
    if (!activeDatabaseId) { openCreateModal(); return; }
  ...
  const updateDetailProperty = (id, field, value) => setDetailProps(prev => prev.map(p => p.id === id ? { ...p, [field]: value } : p));
  const addDetailProperty = () => setDetailProps(prev => [...prev, { id: `new-${Date.now()}`, name: "", property_type: "text", default_value: "" }]);
  const removeDetailProperty = (id) => setDetailProps(prev => prev.length > 1 ? prev.filter(p => p.id !== id) : prev);

  // Save edits
  const saveTemplateEdits = async () => {
    if (!selectedTemplate) return;
    const cleanName = detailName.trim();
    if (!cleanName) { Alert.alert("Validation", "Template name cannot be empty."); return; }
    try {
  ...
  const requestDeleteTemplate = () => {
    if (!canDelete) { Alert.alert("Permission", "Only admins can delete templates."); return; }
    if (!selectedTemplate) return;
    setDeleteModalVisible(true);
  };

  const performDeleteTemplate = async () => {
    if (!selectedTemplate) { setDeleteModalVisible(false); return; }
    if (!canDelete) { Alert.alert("Permission", "Only admins can delete templates."); setDeleteModalVisible(false); return; }
    if (!activeDatabaseId) { openCreateModal(); setDeleteModalVisible(false); return; }
  ...
  // Main grid
  const mainListData = useMemo(() => [{ _type: "add", id: "__add__" }, ...filteredTemplates], [filteredTemplates]);
  ...
  const renderMainItem = ({ item }) => {
    ...
    return (
      ...
      >
        <View style={{ flex: 1 }}>
          <View style={[{ marginTop: "auto", maxWidth: "100%" }, stylesLocal.nameTextWrap]}>
            <AutoShrinkText
              style={[{ color: colors.primary }, stylesLocal.nameText]}
  ...
  return (
    <View style={commonStyles.contentContainer}>
      ...
      <FlatList
        key={listKey}
        data={mainListData}
        keyExtractor={(item) => (item._type === "add" ? "__add__" : String(item.id))}
        renderItem={renderMainItem}
        numColumns={numColumns}
        columnWrapperStyle={numColumns > 1 ? stylesLocal.rowWrap : null}
        contentContainerStyle={numColumns === 1 ? stylesLocal.rowWrap : undefined}
        showsVerticalScrollIndicator={false}
        keyboardShouldPersistTaps="handled"
        ListEmptyComponent={<Text style={{ color: "#888", marginTop: 12 }}>No templates found.</Text>}
      ...
      <NewLogTemplateModal
        ...
        onTemplateNameChange={(value) => { setTemplateName(value); setNameTouched(true); }}
      ...
      <ViewLogTemplatesModal
        ...
        onClose={() => { setDetailsVisible(false); setSelectedTemplate(null); }}
      ...
      >
        ...
        <ModalSmall.Footer>
          <Pressable onPress={() => setDeleteModalVisible(false)} disabled={deletingTemplate} style={modalStyles.cancelButton}>
            <Text style={modalStyles.cancelText}>Cancel</Text>
          </Pressable>
          <Pressable
            onPress={performDeleteTemplate}
            disabled={deletingTemplate}
            style={[modalStyles.primaryButton, { backgroundColor: "#dc2626" }, deletingTemplate && modalStyles.primaryButtonDisabled]}
          >
            <Text style={modalStyles.primaryButtonText}>{deletingTemplate ? "Deleting…" : "Delete"}</Text>
          </Pressable>
        </ModalSmall.Footer>
      </ModalSmall>
    </View>
...
const stylesLocal = StyleSheet.create({
  ...
  nameText: { fontWeight: "700" },        // per-screen typography
  nameTextWrap: { flex: 1, justifyContent: "flex-end" }, // pin to bottom
});
