...
/* =========================
   ...
   ========================= */
const DOCS_BUCKET = "linkedDocs";

const FieldLabel = ({ children, style }) => (
  <Text style={[{ fontSize: 12, color: "#6b7280", marginBottom: 6 }, style]}>
    {children}
  </Text>
);
...
export default function AddLinkedDocModal({ visible, onClose, onCreated }) {
  ...
  useEffect(() => {
    if (visible && !activeDatabaseId) {
      openCreateModal();
      onClose?.();
    }
  }, [visible, activeDatabaseId, openCreateModal, onClose]);
  ...
  const { values: allValueSuggestions, loading: loadingVals } =
    usePropertyValuesSuggestions({
  ...
  useEffect(() => {
    if (!visible || !activeDatabaseId) return;
    (async () => {
      setLoadingTpl(true);
      ...
      else console.error("load templates error", error);
    })();
  }, [visible, activeDatabaseId]);
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      else console.error("load template properties error", error);
    })();
  }, [templateId]);
  ...
  const templateOptions = useMemo(() => {
    ...
    const list = q
      ? base.filter((t) => (t.name || "").toLowerCase().includes(q) || t.id.includes(q))
      : base;
          ...
          {templateId ? (
            <>
              ...
              <AutocompleteInput
                value={
                  propertyId
                    ? properties.find((p) => p.id === propertyId)?.property_name ?? propertyId
                    : propertyQuery
                }
                onChangeText={(t) => {
                  setPropertyQuery(t);
          ...
          <View style={{ flexDirection: "row", justifyContent: "flex-end", gap: 8, marginTop: 6 }}>
            ...
            <Pressable
              ...
              style={{
                ...
                borderRadius: 10,
                backgroundColor: !canConfirm || saving ? "#9ca3af" : "#6366f1",
              }}
            >
